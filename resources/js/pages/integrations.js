...
(function () {
    const CFG = window.__integrationsConfig || {};
    const TWILIO_SETUP = {
        voiceWebhook: CFG.voiceWebhook || '',
        smsWebhook: CFG.smsWebhook || '',
        phoneSystemUrl: CFG.phoneSystemUrl || '',
        viberChatUrl: CFG.viberChatUrl || '',
        whatsappChatUrl: CFG.whatsappChatUrl || '',
        facebookChatUrl: CFG.facebookChatUrl || '',
    };
    ...
    async function loadIntegrationStatus(integrationId) {
        ...
        if (integrationId === 'calendar') {
            try {
                const response = await fetch(CFG.calendarOauthSettingsUrl || '/api/calendar/oauth-settings');
                if (!response.ok) return null;
                const data = await response.json();
    ...
    function getIntegrationConfig(integrationId, existingData = null) {
        const configs = {
            ...
            'stripe': `
                ...
                <div class="webhook-setup-section" style="background: var(--bg-card); border: 1px solid var(--border); border-radius: 8px; padding: 1rem; margin-top: 1rem;">
                    ...
                    <ol style="margin: 0; padding-left: 1.25rem; font-size: 0.8125rem; color: var(--text-secondary); line-height: 1.7;">
                        ...
                        <li>Enter your webhook URL: <code style="background: var(--bg-primary); padding: 0.2em 0.4em; border-radius: 4px; font-size: 0.8em; word-break: break-all;">${CFG.stripeWebhookUrl || ""}</code></li>
                    ...
                    <p style="margin: 0.75rem 0 0; font-size: 0.75rem; color: var(--text-muted);">Your webhook URL: <strong>${CFG.stripeWebhookUrl || ""}</strong></p>
                </div>
            `,
            'gmail': `
                <div class="form-group">
            ...
            'wise': `
                ...
                ${CFG.canViewWiseRecipients ? `
                <div class="form-group">
                    <a href="${CFG.wiseRecipientsUrl || '/wise-recipients'}" class="btn-secondary" style="display:inline-flex;text-decoration:none;">Manage recipients &amp; employees</a>
                ...
                <div class="oauth-section" style="margin-bottom: 1.5rem;">
                    ...
                    <details class="oauth-steps" style="background: var(--bg-primary); border: 1px solid var(--border); border-radius: 8px; padding: 0.75rem 1rem; margin-bottom: 1rem;">
                        ...
                        <ol style="margin: 0.75rem 0 0; padding-left: 1.25rem; font-size: 0.8125rem; color: var(--text-secondary); line-height: 1.6;">
                            ...
                            <li>Add Authorized redirect URI: <code id="calendarGoogleRedirectUrl" style="background: var(--bg-card); padding: 0.125rem 0.375rem; border-radius: 4px; font-size: 0.75rem; word-break: break-all;">${(existingData && existingData.redirect_url_google) ? existingData.redirect_url_google : '${CFG.googleCalendarCallbackUrl || "/calendar/connect/google/callback"}'}</code></li>
    ...
    async function handleStripeSave() {
        ...
        try {
            ...
            const response = await fetch(CFG.stripeStoreUrl || '/api/integrations/stripe', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json',
    ...
    async function handleCalendarOauthSave(provider = 'google') {
        ...
        try {
            const response = await fetch(CFG.calendarOauthSettingsStoreUrl || '/api/calendar/oauth-settings', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json',
    ...
    if (typeof closeIntegrationModal === 'function') window.closeIntegrationModal = closeIntegrationModal;
    if (typeof handleFrontCommentImport === 'function') window.handleFrontCommentImport = handleFrontCommentImport;
    if (typeof handleFrontCommentResetProgress === 'function') window.handleFrontCommentResetProgress = handleFrontCommentResetProgress;
    if (typeof handleFrontDiscussionImport === 'function') window.handleFrontDiscussionImport = handleFrontDiscussionImport;
    if (typeof handleFrontDiscussionResetProgress === 'function') window.handleFrontDiscussionResetProgress = handleFrontDiscussionResetProgress;
    if (typeof handleFrontImport === 'function') window.handleFrontImport = handleFrontImport;
    if (typeof handleFrontResetProgress === 'function') window.handleFrontResetProgress = handleFrontResetProgress;
    if (typeof handleIntegrationAction === 'function') window.handleIntegrationAction = handleIntegrationAction;
    if (typeof openIntegrationModal === 'function') window.openIntegrationModal = openIntegrationModal;
    if (typeof syncFacebookHistory === 'function') window.syncFacebookHistory = syncFacebookHistory;
})();
