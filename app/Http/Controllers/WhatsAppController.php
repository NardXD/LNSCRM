...
class WhatsAppController extends Controller
{
    ...
    public function sync(Request $request): JsonResponse
    {
        ...
        usort($messages, function ($a, $b) {
            return $this->twilioMessageTimestamp($a) <=> $this->twilioMessageTimestamp($b);
        });
        ...
        foreach ($messages as $message) {
            ...
            try {
                if ($this->importSyncedWhatsAppMessage($integration, $twilio, $ownNumber, $message)) {
                    $imported++;
                } else {
                    $skipped++;
                }
    ...
    protected function importSyncedWhatsAppMessage(
        ...
        $conversation = $this->upsertConversation($integration->company_id, $peer, null);
        ...
        if ($numMedia > 0) {
            try {
                ...
                if ($media) {
                    ...
                    $mediaUrl = $this->storeInboundMedia($integration, $media['url'], $mimeType, null, (string) $message->sid);
                }
            } catch (\Throwable $e) {
        ...
        $sentAt = $this->twilioMessageDate($message);
    ...
    protected function twilioMessageDate(object $message): ?\Carbon\Carbon
    {
        foreach ([$message->dateSent ?? null, $message->dateCreated ?? null] as $candidate) {
            ...
            try {
                if ($candidate instanceof \DateTimeInterface) {
                    return \Carbon\Carbon::instance($candidate)->timezone(config('app.timezone'));
                }

                return \Carbon\Carbon::parse((string) $candidate)->timezone(config('app.timezone'));
            } catch (\Throwable) {
                continue;
            }
        }
    ...
    protected function twilioMessageTimestamp(object $message): int
    {
        return $this->twilioMessageDate($message)?->getTimestamp() ?? 0;
    }
